...
class PersonnelJadwalApiTest extends TestCase
{
    ...
    public function test_jadwal_endpoint_returns_monthly_calendar_and_shift_details(): void
    {
        ...
        $this->assertFalse($jadwals[9]['is_dinas']);

        // Hari ke-11 (index 10) - Libur
        ...
        $this->assertFalse($jadwals[10]['is_dinas']);

        // Hari ke-12 (index 11) - Dinas
        $this->assertEquals('2026-05-12', $jadwals[11]['tanggal']);
        $this->assertEquals('D', $jadwals[11]['shift_name']);
        $this->assertEquals('DINAS', $jadwals[11]['status']);
        $this->assertNull($jadwals[11]['shift_hours']);
        $this->assertFalse($jadwals[11]['is_off']);
        $this->assertTrue($jadwals[11]['is_dinas']);
