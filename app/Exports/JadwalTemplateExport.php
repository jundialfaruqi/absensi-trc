...
class JadwalTemplateExport implements FromCollection, WithHeadings, ShouldAutoSize, WithTitle, WithStyles, WithColumnWidths, WithEvents
{
    protected $month;
    protected $year;
    protected $opdId;
    ...
    public function headings(): array
    {
        ...
        $dateNumbers = ['', '']; // Space for merged ID/Name labels
        ...
        return [
            ['TEMPLATE IMPORT JADWAL SHIFT'],
            ['Periode:', Carbon::create($this->year, $this->month, 1)->translatedFormat('F Y')],
            ['Instruksi:', 'JANGAN MENGUBAH FORMAT (Hapus/Tambah Kolom). Isi tanggal dengan NAMA SHIFT. Scroll ke bawah untuk daftar shift.'],
            [''], 
            $dayNames,     // Row 5
            $dateNumbers,  // Row 6
        ];
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('B3')->getFont()->setBold(true)->getColor()->setARGB('FFFF0000'); // RED warning

        // Merge ID and Name cells vertically for the two-row header
        $sheet->mergeCells('A5:A6');
        $sheet->mergeCells('B5:B6');
        $sheet->getStyle('A5:B6')->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);

        // Header Style (Row 5 & 6)
        ...
        // Red Color for Weekends in Day Names
        for ($i = 1; $i <= $daysInMonth; $i++) {
            $date = Carbon::create($this->year, $this->month, $i);
            if ($date->isWeekend()) {
                $col = Coordinate::stringFromColumnIndex($i + 2);
                $sheet->getStyle($col . '5')->getFont()->getColor()->setARGB('FFFCA5A5'); // Light red
            }
        }

        // --- STYLE SHIFT REFERENCE (Bottom Section) ---
        $refStartRow = 6 + $this->personnelCount + 3; // 6(header) + count + 2(spacer) + 1(start)
        ...
        $sheet->getStyle('A' . $refHeaderRow . ':C' . $refHeaderRow)->applyFromArray([
            'font' => ['bold' => true, 'color' => ['argb' => 'FFFFFFFF']],
            'fill' => ['fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID, 'startColor' => ['argb' => 'FF059669']], // Green
        ]);
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                // Align center for ID Personnel (Column A)
                ...
                // Align center and Wrap Text for date columns (C onwards)
    ...
    {
        return 'Template Jadwal ' . Carbon::create($this->year, $this->month, 1)->format('M Y');
    }
}
