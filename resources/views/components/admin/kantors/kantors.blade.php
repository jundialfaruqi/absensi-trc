    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
        ...
        <div class="text-sm breadcrumbs text-base-content/60">
            <ul>
                ...
                <li>
                    <a href="{{ route('kantor') }}">
                        <span class="text-base-content">Manajemen Kantor</span>
                    </a>
                </li>
            </ul>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="table table-zebra w-full">
                    ...
                    <tbody>
                        @forelse ($this->kantors as $r)
                            <tr class="hover:bg-base-200/50">
                                ...
                                <td>
                                    ...
                                    <div class="text-[10px] text-primary font-mono mt-1">{{ $r->latitude }}, {{ $r->longitude }}</div>
                                </td>
                                <td>
                                    <div class="badge badge-outline badge-sm">{{ $r->opd->name }}</div>
                                </td>
                                <td>
                                ...
                                <td class="text-center">
                                    <div class="dropdown dropdown-left dropdown-end">
                                        ...
                                        <ul tabindex="0" class="dropdown-content menu p-2 shadow-md bg-base-100 rounded-box w-36 z-50">
                                            <li><button type="button" wire:click="openEditModal({{ $r->id }})">Edit</button></li>
                                            <li><button type="button" class="text-error" wire:click="confirmDelete({{ $r->id }}, '{{ addslashes($r->name) }}')">Delete</button></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="text-center text-sm text-base-content/60 py-8">Tidak ada data Kantor</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <dialog id="kantor-modal" class="modal backdrop-blur-xs" wire:ignore.self
        ...
        <div class="modal-box shadow w-11/12 max-w-5xl p-0 overflow-hidden">
            <div class="p-6 border-b border-base-200 bg-base-200/30 flex justify-between items-center">
                ...
                <button type="button" class="btn btn-ghost btn-sm btn-circle" onclick="document.getElementById('kantor-modal').close()">✕</button>
            </div>
            
            <form wire:submit="save" class="flex flex-col md:flex-row h-auto md:h-[600px]">
                {{-- Left Side: Form --}}
                <div class="w-full md:w-1/3 p-6 space-y-4 overflow-y-auto border-r border-base-200">
                    <div class="form-control">
                        <label class="label"><span class="label-text font-bold">Nama Kantor</span></label>
                        <input type="text" wire:model="name" class="input input-bordered w-full" placeholder="Cth: Kantor Pusat TRC">
                        @error('name') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-control">
                        <label class="label"><span class="label-text font-bold">OPD Induk</span></label>
                        <select wire:model="opd_id" class="select select-bordered w-full" @disabled(!auth()->user()->hasRole('super-admin'))>
                    ...
                    <div class="form-control">
                        <label class="label"><span class="label-text font-bold">Alamat</span></label>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div class="form-control">
                            <label class="label"><span class="label-text font-bold text-[10px] uppercase">Latitude</span></label>
                            <input type="text" wire:model="latitude" class="input input-bordered input-sm font-mono" readonly>
                            @error('latitude') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                        <div class="form-control">
                            <label class="label"><span class="label-text font-bold text-[10px] uppercase">Longitude</span></label>
                            <input type="text" wire:model="longitude" class="input input-bordered input-sm font-mono" readonly>
                    ...
                    <div class="form-control">
                        <label class="label flex justify-between">
                            <span class="label-text font-bold">Radius Absensi</span>
                            <span class="text-xs font-black text-primary">{{ $radius_meter }} Meter</span>
                        </label>
                        <input type="range" min="50" max="1000" step="10" wire:model.live="radius_meter" class="range range-primary range-xs">
                        <div class="w-full flex justify-between text-[10px] px-2 mt-1 opacity-50 uppercase font-bold">
                            <span>50m</span>
                            <span>1km</span>
                        </div>
                        @error('radius_meter') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-control">
                        <label class="label cursor-pointer justify-start gap-4">
                            <input type="checkbox" wire:model="is_active" class="checkbox checkbox-success">
                            <span class="label-text font-bold">Kantor Aktif</span>
                        </label>
                    </div>
                ...
                {{-- Right Side: Map --}}
                <div class="w-full md:w-2/3 h-[400px] md:h-full relative bg-base-300" wire:ignore>
                    <div id="map-selection" class="w-full h-full"></div>
                    <div class="absolute top-4 left-1/2 -translate-x-1/2 z-[1000] pointer-events-none">
                        <div class="badge badge-neutral p-4 shadow-xl border-none opacity-90 text-[10px] uppercase font-black tracking-widest gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4 text-primary">
    ...
    <dialog id="kantor-delete-modal" class="modal"
        ...
        <div class="modal-box">
            ...
            <div class="modal-action">
                <button type="button" class="btn" onclick="document.getElementById('kantor-delete-modal').close()">Batal</button>
                <button type="button" class="btn btn-error text-white" wire:click="executeDelete" wire:loading.attr="disabled">
