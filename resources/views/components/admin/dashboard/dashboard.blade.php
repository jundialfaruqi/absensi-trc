<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2 px-4 py-2 bg-base-100 rounded-xl border border-base-300 shadow-sm">
            ...
            <span class="text-[10px] font-black uppercase tracking-widest text-base-content opacity-70">Sistem Berjalan Normal</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {{-- Total Personnel --}}
        <div class="stats shadow bg-base-100 border border-base-200 group hover:border-primary/30 transition-all duration-300">
            <div class="stat">
                <div class="stat-figure text-primary opacity-20 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                    </svg>
                </div>
                <div class="stat-title text-[10px] font-black uppercase tracking-widest text-base-content/50">Total Personel</div>
        ...
        {{-- Absen Masuk --}}
        <div class="stats shadow bg-base-100 border border-base-200 group hover:border-success/30 transition-all duration-300">
            <div class="stat">
                <div class="stat-figure text-success opacity-20 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M11.5 21l-5.5-2V7l5.5 2V21zM11.5 21l5.5-2V7l-5.5 2V21zM9.5 13H13.5M11.5 11V15" />
                    </svg>
                </div>
                <div class="stat-title text-[10px] font-black uppercase tracking-widest text-base-content/50">Absen Masuk</div>
                <div class="stat-value text-success">{{ number_format($stats['total_masuk']) }}</div>
                <div class="stat-desc font-bold text-[9px] uppercase tracking-tighter mt-1">{{ $stats['hadi_percentage'] }}% dari total personel</div>
            </div>
        </div>

        {{-- Absen Pulang --}}
        <div class="stats shadow bg-base-100 border border-base-200 group hover:border-secondary/30 transition-all duration-300">
            <div class="stat">
                <div class="stat-figure text-secondary opacity-20 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
                    </svg>
                </div>
                <div class="stat-title text-[10px] font-black uppercase tracking-widest text-base-content/50">Selesai Tugas</div>
        ...
        {{-- Terlambat --}}
        <div class="stats shadow bg-base-100 border border-base-200 group hover:border-error/30 transition-all duration-300">
            <div class="stat">
                <div class="stat-figure text-error opacity-20 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </div>
                <div class="stat-title text-[10px] font-black uppercase tracking-widest text-base-content/50">Terlambat</div>
                <div class="stat-value text-error">{{ number_format($stats['total_terlambat']) }}</div>
                <div class="stat-desc font-bold text-[9px] uppercase tracking-tighter mt-1 text-error">Memerlukan perhatian</div>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-8">
            ...
            <div class="card bg-base-100 shadow-xl border border-base-200 overflow-hidden">
                <div class="p-4 bg-base-200/50 border-b border-base-200 flex items-center justify-between">
                    <h3 class="text-xs font-black text-base-content/70 uppercase tracking-widest italic">Aksi Cepat: Cuti</h3>
                    @if($stats['pending_leaves_count'] > 0)
                        <span class="badge badge-error badge-xs font-black text-[8px]">{{ $stats['pending_leaves_count'] }}</span>
                    @endif
                </div>
                    ...
                    @forelse($pendingLeaves as $leave)
                        <div class="flex flex-col p-3 rounded-xl bg-base-200/30 border border-base-200 group relative">
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex flex-col">
                                    <span class="text-[11px] font-black text-base-content uppercase tracking-tight">{{ $leave->personnel->name }}</span>
                                    <span class="text-[9px] font-bold text-primary uppercase">{{ $leave->cuti->name }}</span>
                                </div>
                            ...
                            <div class="flex items-center gap-2 text-[9px] font-bold text-base-content/40 uppercase">
                                ...
                                {{ $leave->tanggal_mulai->format('d/m') }} - {{ $leave->tanggal_selesai->format('d/m') }}
                                <span>({{ $leave->tanggal_mulai->diffInDays($leave->tanggal_selesai) + 1 }} Hari)</span>
                            </div>
                        </div>
                    @empty
                        <div class="py-6 text-center">
                            <div class="w-10 h-10 rounded-full bg-base-200 flex items-center justify-center mx-auto mb-2 opacity-20">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                            </div>
                            <p class="text-[10px] font-black text-base-content/20 uppercase tracking-widest">Antrian Kosong</p>
                        </div>
                    @endforelse
                </div>
                @if($stats['pending_leaves_count'] > 0)
                    <div class="p-3 bg-base-200/50 border-t border-base-200">
                        <a href="{{ route('permohonan-cuti') }}" class="btn btn-ghost btn-xs btn-block text-[9px] font-black uppercase tracking-widest opacity-60 hover:opacity-100">
            ...
            <div class="card bg-base-100 shadow-xl border border-base-200 p-6 space-y-4">
                ...
                <progress class="progress progress-primary w-full h-1.5" value="{{ $stats['hadi_percentage'] }}" max="100"></progress>
                
                <div class="grid grid-cols-2 gap-4 pt-2">
                    <div class="p-3 bg-base-200/50 rounded-xl border border-base-200">
                        <span class="block text-[8px] font-black text-base-content/40 uppercase tracking-widest mb-1">Masuk</span>
                        <span class="text-lg font-black text-success">{{ $stats['total_masuk'] }}</span>
                    </div>
                    <div class="p-3 bg-base-200/50 rounded-xl border border-base-200">
                        <span class="block text-[8px] font-black text-base-content/40 uppercase tracking-widest mb-1">Pulang</span>
            ...
            <div class="card bg-primary text-primary-content shadow-xl p-6 relative overflow-hidden group">
                ...
                <div class="relative z-10 space-y-3">
                    <h3 class="text-lg font-black uppercase italic tracking-tighter leading-none">Bantuan & <br>Informasi</h3>
                    ...
                    <button class="btn btn-sm bg-white/20 hover:bg-white/30 border-none text-white text-[10px] font-black uppercase tracking-widest mt-2">
                        Pusat Bantuan
                    </button>
                </div>
            </div>
