<div>
                    ...
                    <div class="flex justify-between items-start">
                        ...
                        <div class="p-2 bg-base-200 rounded-lg">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 14c3.866 0 7 1.343 7 3v1H5v-1c0-1.657 3.134-3 7-3z" />
                    ...
                    <tbody>
                        @forelse ($this->opds as $r)
                            <tr class="hover:bg-base-200/50">
                                ...
                                <td>
                                    @if($r->singkatan)
                                        <div class="badge badge-primary badge-outline badge-sm border-none bg-primary/10 text-primary font-semibold">{{ $r->singkatan }}</div>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td>
                                    <div class="flex -space-x-2 overflow-hidden items-center">
                                        @foreach($r->users->take(3) as $u)
                                            <div class="bg-primary text-primary-content rounded-full w-8 h-8 flex items-center justify-center text-xs font-bold ring ring-base-100" title="{{ $u->name }}">
                                                {{ substr($u->name, 0, 1) }}
                                            </div>
                                        @endforeach
                                        @if($r->users_count > 3)
                                            <div class="bg-base-300 text-base-content rounded-full w-8 h-8 flex items-center justify-center text-xs ring ring-base-100">
                                                +{{ $r->users_count - 3 }}
                                            </div>
                                        @endif
                                        @if($r->users_count === 0)
                                ...
                                <td class="text-center">
                                    <div class="dropdown dropdown-left dropdown-end">
                                        ...
                                        <ul tabindex="0" class="dropdown-content menu p-2 shadow-md bg-base-100 rounded-box w-36 z-50">
                                            ...
                                            <li>
                                                <button type="button" class="text-error" wire:click="confirmDelete({{ $r->id }}, '{{ addslashes($r->name) }}')">
                                                    Delete
                                                </button>
                                            </li>
                                        </ul>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center text-sm text-base-content/60 py-8">Tidak ada data OPD</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="modal-box shadow w-11/12 max-w-3xl">
            ...
            <form wire:submit="save">
                <div class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="form-control w-full">
                            ...
                            <input type="text" wire:model="name" class="input input-bordered focus:input-primary w-full transition-all @error('name') input-error @enderror" placeholder="Cth: Dinas Komunikasi dan Informatika">
                        ...
                        <div class="form-control w-full">
                            ...
                            <input type="text" wire:model="singkatan" class="input input-bordered focus:input-primary w-full transition-all @error('singkatan') input-error @enderror" placeholder="Cth: Diskominfo">
                            @error('singkatan') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div class="form-control w-full">
                        ...
                        <textarea wire:model="alamat" class="textarea textarea-bordered focus:textarea-primary w-full transition-all h-20 @error('alamat') textarea-error @enderror" placeholder="Masukkan alamat lengkap instansi..."></textarea>
                    ...
                    <div class="form-control w-full">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                            <div class="w-full sm:flex-1">
                                <input type="file" wire:model="logo" class="file-input file-input-bordered focus:file-input-primary transition-all w-full @error('logo') file-input-error @enderror" accept="image/*" />
                                @error('logo') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                <div wire:loading wire:target="logo" class="mt-2 text-xs text-info font-medium italic">Mengunggah file...</div>
                            </div>
                            
                            {{-- Preview section --}}
                            @if ($logo)
                                <div class="avatar p-1 border border-base-200 rounded-lg shadow-sm bg-base-100 flex-shrink-0">
                                    <div class="w-16 h-16 rounded">
                                        <img src="{{ $logo->temporaryUrl() }}" class="object-contain bg-white">
                                    </div>
                                </div>
                            @elseif ($oldLogo)
                                <div class="avatar p-1 border border-base-200 rounded-lg shadow-sm bg-base-200/50 flex-shrink-0">
                                    <div class="w-16 h-16 rounded">
                    ...
                    <div class="form-control w-full">
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-end mb-2 gap-2 px-1">
                            <div>
                                <label class="label p-0 pb-1"><span class="label-text font-medium">Pengguna Terkait</span></label>
                                <p class="text-xs text-base-content/60">Pilih pengguna. (Otomatis dilepas dari OPD lain jika ditambahkan kesini)</p>
                            </div>
                            <div class="relative w-full sm:w-64">
                                <input type="text" wire:model.live.debounce.300ms="userSearch" placeholder="Cari nama pengguna..." class="input input-sm input-bordered w-full pl-8 focus:input-primary transition-all">
                                <span class="absolute inset-y-0 left-0 flex items-center pl-2.5 pointer-events-none text-base-content/50">
                            ...
                            <div class="max-h-56 overflow-y-auto p-3">
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5">
                                    @forelse ($this->allUsers as $u)
                                        <label class="label cursor-pointer justify-start gap-3 hover:bg-base-100 hover:shadow-sm p-2 rounded-lg transition-all border border-transparent hover:border-base-content/10 select-none">
                                            <input type="checkbox" wire:model="selectedUsers" value="{{ $u->id }}" class="checkbox checkbox-sm checkbox-primary" />
                                            <span class="label-text truncate font-medium text-sm">{{ $u->name }}</span>
                                        </label>
                                    @empty
                                        <div class="col-span-full flex flex-col items-center justify-center text-center py-6 text-base-content/50">
                                            ...
                                            <span class="text-sm">{{ $userSearch ? 'Tidak ada pengguna yang dicari.' : 'Belum ada pengguna.' }}</span>
                                        </div>
                                    @endforelse
                                </div>
                            </div>
                ...
                <div class="modal-action mt-6">
                    <button type="button" class="btn btn-ghost" x-on:click="document.getElementById('opd-modal').close()">Batal</button>
    ...
    <dialog id="opd-delete-modal" class="modal"
        ...
        <div class="modal-box">
            ...
            <p class="text-xs text-error mt-2 italic">Pengguna yang terhubung dengan OPD ini tidak akan dihapus, hanya dilepaskan dari OPD ini.</p>
            <div class="modal-action">
                <button type="button" class="btn" x-on:click="document.getElementById('opd-delete-modal').close()">Batal</button>
    ...
    <div class="fab fab-flower fab-bottom fab-end mb-12 sm:hidden relative z-40">
        ...
        <button type="button" class="tooltip btn btn-circle btn-neutral" wire:click="openAddModal" data-tip="Tambah OPD">
