    ...
    <div class="space-y-8">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @forelse($beritas as $berita)
                <div
                    class="glass-panel rounded-4xl hover:border-blue-500/30 transition-all group overflow-hidden relative flex flex-col h-full border border-white/5">
                    {{-- Image --}}
                    ...
                    <div class="p-6 flex flex-col grow">
                        ...
                        <a href="{{ route('public.berita.show', $berita->slug) }}" class="block group/title">
                            <h3 class="text-xl font-black text-white leading-tight mb-3 line-clamp-2 group-hover/title:text-blue-400 transition-colors"
                                title="{{ $berita->judul }}">
                                {{ $berita->judul }}
                            </h3>
                        </a>
                        ...
                        <div class="mt-auto pt-4 border-t border-white/5">
                            <a href="{{ route('public.berita.show', $berita->slug) }}"
                                class="inline-flex items-center gap-2 text-xs font-bold text-white hover:text-blue-400 transition-colors uppercase tracking-widest group/btn">
                                BACA SELENGKAPNYA
        ...
        <div wire:loading wire:target="loadMore" class="w-full mt-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @for ($i = 0; $i < 3; $i++)
                    <div
                        class="glass-panel rounded-4xl overflow-hidden relative flex flex-col h-full border border-white/5 animate-pulse">
        ...
        @if ($total > $amount)
            <div class="flex justify-center mt-8">
                <button wire:click="loadMore" wire:loading.attr="disabled"
                    class="px-8 py-3 glass-panel hover:bg-white/5 border border-white/10 rounded-xl text-white font-bold tracking-widest text-sm transition-all uppercase flex items-center gap-2 group relative overflow-hidden">
                    <span class="relative z-10 flex items-center gap-2" wire:loading.remove wire:target="loadMore">
                        MUAT LAINNYA
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="h-4 w-4 group-hover:translate-y-1 transition-transform" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
